...
public class PayInCardAdvanceQueryHandler implements AdvanceQueryHandler {
    ...
    public boolean support(TransactionAdvanceQueryRequest request) {
        return TransactionType.PAY_IN.equals(request.getTransactionType()) &&
                ProductCode.CARD.equals(request.getProductCode());
    }
    ...
    private List<String> queryPayinNormalData(TransactionAdvanceQueryRequest request) {
        ...
        if (normalConfig != null && !normalConfig.isIgnore()) {
            Criteria criteria = Criteria.where("after.merchant_name")
                    ...
                    .in(PaymentMethodCode.CREDIT_CARD.name(), PaymentMethodCode.CARD.name())
                    .and("after.country").is(request.getCountryCode());
            if (request.getOther().containsKey("token")) {
                criteria.and("after.payment_info").regex(request.getOther().get("token"));
            }
    ...
    private List<String> queryPayinBackupData(TransactionAdvanceQueryRequest request) {
        ...
        if (backupConfig != null && !backupConfig.isIgnore()) {
            Criteria criteria = Criteria.where("source.db").is(backupConfig.getDb())
