...
public abstract class ShopifyMapper {

    @Mapping(target = "paymentId", source = "after.id")
    ...
    public abstract ShopifyOrderItem toEventLog(final ShopifyEventLog vo);

    @AfterMapping
    void afterMapping(@MappingTarget final ShopifyOrderItem target, final ShopifyEventLog source) {
        // product code
        if (PaymentLinkPaymentMethod.BANK_TRANSFER_BR.name()
            ...
            target.setProductCode(
                    ProductCode.valueOf(source.getAfter().getPayment_method()).name());
        }
        ...
        target.setAmount(new BigDecimal(source.getAfter().getAmount()).multiply(new BigDecimal(100))
                .longValue());
    ...
    public abstract List<ShopifyOrderItem> toEventLogList(final List<ShopifyEventLog> voList);
}
